...
FYI: You, Claude Code, manage persistent memory using two main file types: `AGENTS.md` for shared or global project context,
...
To help you navigate the project, here is an overview of the main directories:

```
assetextractor/
├── extraction/      # RDA file extraction
├── parsing/
│   └── core/        # Core XML parsing and object model
├── conversion/      # Data conversion scripts (e.g., to HTML)
│   └── assetbrowser/
├── versioning/      # Asset versioning and database logic
tests/
├── integration/     # Integration tests
└── debugging/       # Standalone debugging scripts
.claude/
└── commands/        # Commands for the AI assistant
```
---
...
For example:
* Information A belongs exclusively to FastAPI backend code → put it in `backend/AGENTS.md`
* Information B belongs exclusively to TypeScript/Electron frontend → put it in `frontend/AGENTS.md`
* Information C belongs exclusively to testing → put it in `tests/AGENTS.md`
* Information D belongs exclusively to ML models/TabPFN-TS → put it in `backend/ml_models/AGENTS.md`
* Information E belongs exclusively to documentation → put it in `documentation/AGENTS.md`
* Information about RDA file extraction logic → put it in `assetextractor/extraction/AGENTS.md`
* Information about the core parsing logic (Assets, Templates, etc.) → put it in `assetextractor/parsing/core/AGENTS.md`
* Information about asset conversion scripts → put it in `assetextractor/conversion/AGENTS.md`
* Information about asset versioning and the database schema → put it in `assetextractor/versioning/AGENTS.md`
* Information about testing fixtures or methodologies → put it in `tests/AGENTS.md`

If you added a new command that developpers need to run / test / debug the application → update `README.md`

If you create a new `AGENTS.md`, create a `CLAUDE.md` in the same folder with the following content:
```
<!--
SPDX-FileCopyrightText: 2025 Bayreuth AI Association <info@bayreuth-ai.de>
...
**Important**: Keep the information concise and compact. Provide a short, result-oriented **summary** of the final architecture and encountered pitfalls.
